...
const presets: Record<WritingStudioSubject, Record<WritingStudioMode, WritingStudioPreset>> = {
  chinese: {
    idea: {
      badge: '语文写作思路',
      title: '作文审题与立意工作台',
      description: '围绕题目快速生成立意方向、结构框架、素材调用建议和开头切入点。',
      taskLabel: '作文题目 / 写作任务',
      ...
      actions: [
        { id: 'idea-review', label: '审题立意', prompt: '先帮我拆题，判断写作任务、核心立意和容易跑题的风险。' },
        { id: 'idea-outline', label: '文章结构', prompt: '请给我一版清晰可写的文章结构，并说明每一段该写什么。' },
        { id: 'idea-materials', label: '素材建议', prompt: '请结合题目推荐可用素材、细节和情感转折点。' },
        { id: 'idea-opening', label: '开头结尾', prompt: '请给我开头切入和结尾收束的写法建议。' },
      ],
    ...
    review: {
      ...
      actions: [
        { id: 'review-overall', label: '总评定位', prompt: '请先给我整体评价，判断立意、结构和表达最需要优先改哪一块。' },
        { id: 'review-structure', label: '结构问题', prompt: '请重点检查段落层次、叙事推进和首尾呼应是否成立。' },
        { id: 'review-language', label: '语言表达', prompt: '请重点指出语言表达、病句和重复啰嗦的问题，并给替换建议。' },
        { id: 'review-upgrade', label: '提分改写', prompt: '请给我一轮可直接套用的提分改写建议和下一稿修改顺序。' },
      ],
      accentClass: 'text-red-600',
      badgeClass: 'bg-red-100 text-red-700',
    },
  ...
  english: {
    idea: {
      ...
      taskLabel: '英文写作题目 / 任务',
      taskPlaceholder: '例如：假如你是李华，请写一封邮件邀请交换生参加校园科技节。',
      draftLabel: '补充要求（可选）',
      draftPlaceholder: '例如：考试类型、字数、你最担心的语法点、想突出的人设或观点。',
      tips: [
        '优先把任务场景、收件人、体裁和字数写清楚。',
        '如果想先要提纲，再要高级句型，可以分两次点不同快捷动作。',
        '英语模块这一版先做写作主链，听力入口后续单独接入。',
      ],
      actions: [
        { id: 'english-brief', label: '审题与时态', prompt: '请先判断这道英语写作题的文体、时态、人称和容易失分的地方。' },
        { id: 'english-outline', label: '段落提纲', prompt: '请给我一版三段或四段提纲，并说明每段该写哪些要点。' },
        { id: 'english-phrases', label: '高分表达', prompt: '请给我可直接套用的高级句型、连接词和关键词组。' },
        { id: 'english-opening', label: '首段起笔', prompt: '请帮我写开头切入思路，并给 2 到 3 个可改写的英文起句。' },
      ],
    ...
    review: {
      badge: '英语作文批改',
      title: '英语作文批改工作台',
      description: '检查语法、拼写、表达自然度和篇章衔接，并给出可以直接替换的升级句。',
      ...
      tips: [
        ...
        '后续会再补齐英语听力和更细的阅读/语法专属结果页。',
      ],
      actions: [
        { id: 'english-check', label: '语法纠错', prompt: '请重点找出语法、拼写和基础表达错误，并逐类说明。' },
        { id: 'english-upgrade', label: '表达升级', prompt: '请给我表达升级建议，把句子改得更自然、更像高分作文。' },
        { id: 'english-flow', label: '结构衔接', prompt: '请重点检查段落组织、逻辑衔接和首尾完整性。' },
        { id: 'english-rewrite', label: '改写示例', prompt: '请给我一段示范改写，并列出下一稿修改清单。' },
      ],
      accentClass: 'text-purple-600',
      badgeClass: 'bg-purple-100 text-purple-700',
    },
...
function buildInitialPrompt(
  ...
  if (subject === 'chinese' && mode === 'review') {
    return [
      ...
      `作文草稿：\n${draft}`,
      `本次重点：${action.prompt}`,
      '请按“总体评价、亮点、立意与跑题风险、结构问题、语言表达问题、可直接替换的改写建议、下一稿修改顺序”输出。',
    ]
      .filter(Boolean)
      .join('\n\n');
  }
  ...
  return [
    ...
    `作文草稿：\n${draft}`,
    `本次重点：${action.prompt}`,
    '请按“总体评价、语法与拼写问题、表达升级建议、结构衔接建议、可直接替换的高分句、示范改写、下一稿修改清单”输出。',
  ]
    .filter(Boolean)
    .join('\n\n');
}
...
function buildWritingTitle(subject: WritingStudioSubject, mode: WritingStudioMode, taskInput: string, draftInput: string) {
  const primary = taskInput.trim() || draftInput.trim();
  const prefix = subject === 'chinese'
    ? mode === 'idea' ? '语文写作思路' : '语文作文批改'
    : mode === 'idea' ? '英语写作思路' : '英语作文批改';
...
export function WritingStudio({ subject, mode }: WritingStudioProps) {
  const { profile } = useAuth();
  const preset = presets[subject][mode];
  const sessionIdRef = useRef(`study_${subject}_${mode}_${Date.now()}`);
    ...
    try {
      const nextAssetId = await saveStudyAssetTurn({
        ...
        payload: {
          ...
          taskInput,
          draftInput,
          resultSummary: buildStudyResultSummaryPayload(getWritingModule(subject, mode), assistantContent),
        },
        messages: [
          {
            role: 'user',
      ...
      console.error('[WritingStudio] Failed to save study asset:', error);
    }
  }
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({
          ...
          subjectConfidence: 1,
          questionContent,
          questionType: mode === 'review' || mode === 'idea' ? 'writing' : 'unknown',
          userId: profile?.id,
        }),
      });
      ...
      setMessages((current) => [
        ...
        createMessage(
          'assistant',
          '当前写作工作台请求失败了。可以稍后重试，或先保留题目/草稿继续使用现有录题与 Essay 主链。',
        ),
      ]);
    } finally {
      setIsLoading(false);
  ...
  function handleReset() {
    sessionIdRef.current = `study_${subject}_${mode}_${Date.now()}`;
    setAssetId(null);
    setMessages([]);
    setFollowUpInput('');
    setNotice('');
    ...
    <section className="grid gap-4 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        ...
        <CardHeader className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="hidden rounded-2xl bg-slate-50 p-3 text-slate-500 sm:block">
              {subject === 'english' ? <Languages className="h-5 w-5" /> : <Lightbulb className="h-5 w-5" />}
            </div>
          </div>
        </CardHeader>
      ...
      <Card className="rounded-[28px] border-slate-200 bg-white/90 shadow-sm">
        <CardHeader className="border-b border-slate-200">
          <div className="flex items-center justify-between gap-3">
            <div>
              <CardTitle className="text-xl text-slate-900">写作对话工作区</CardTitle>
              <CardDescription className="mt-2 text-sm leading-6 text-slate-600">
                这一轮先做模块内可用版本，保留既有错题、复习和 Essay 主链不受影响。
              </CardDescription>
            </div>
            <Button
              type="button"
        ...
        <CardContent className="flex min-h-[640px] flex-col px-5 pb-5 pt-5">
          {latestAssistantMessage ? (
            <div className="mb-4">
              <StudyResultSummary
                ...
                title="本轮结果摘要"
                description="把最近一次写作回复提炼成结构化结果卡，便于当前工作台和学习记录详情页回看。"
              />
            </div>
          ) : null}
          ...
          <div className="mt-4 rounded-[24px] border border-blue-200 bg-blue-50/80 p-4 text-sm leading-6 text-slate-700">
            ...
            <p className="mt-2">
              这个写作工作台优先解决“能直接输入并得到分模块输出”的问题。更深的历史沉淀、报告资产和统一 StudySession
              仍会放在下一阶段继续收口。
            </p>
          </div>
        </CardContent>
