...
const actions: ListeningAction[] = [
  {
    ...
    label: '主旨与场景',
    prompt: '请先判断这段听力的场景、人物关系、核心任务和主旨。',
  },
  {
    id: 'listening-answer-locate',
    label: '定位答案',
    prompt: '请结合题目帮我定位答案依据，指出关键词和容易忽略的转折信息。',
  },
  {
...
function buildInitialPrompt(
  ...
  return [
    '请作为英语听力老师，用中文讲解并在必要时保留英文原句，帮助我拆解这道英语听力题。',
    `听力原文、转写稿或老师口述要点：\n${transcript}`,
    question ? `题目：\n${question}` : '',
    answer ? `我的答案或当前判断：\n${answer}` : '',
    note ? `补充信息：\n${note}` : '',
...
export function EnglishListeningStudio() {
  const { profile } = useAuth();
  const sessionIdRef = useRef(`study_english_listening_${Date.now()}`);
    ...
    try {
      const nextAssetId = await saveStudyAssetTurn({
        ...
        payload: {
          ...
          answerInput,
          noteInput,
          resultSummary: buildStudyResultSummaryPayload('listening', assistantContent),
        },
        messages: [
          {
            role: 'user',
      ...
      console.error('[EnglishListeningStudio] Failed to save study asset:', error);
    }
  }
  ...
  async function sendToChatApi(userMessage: string) {
    ...
    try {
      ...
      setMessages((current) => [
        ...
        createMessage(
          'assistant',
          '当前英语听力工作台请求失败了。可以稍后重试，或先保留转写稿继续使用英语录题与写作模块。',
        ),
      ]);
    } finally {
      setIsLoading(false);
  ...
  function handleReset() {
    sessionIdRef.current = `study_english_listening_${Date.now()}`;
    setAssetId(null);
    setMessages([]);
    setFollowUpInput('');
    setNotice('');
    ...
    <section className="grid gap-4 xl:grid-cols-[minmax(0,0.96fr)_minmax(0,1.04fr)]">
          ...
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <CardDescription className="mt-2 text-sm leading-6 text-slate-600">
                当前先用听力原文、转写稿或老师口述要点做题目拆解、答案定位和复练建议，不直接引入新的音频上传链路。
              </CardDescription>
            </div>
      ...
      <Card className="rounded-[28px] border-slate-200 bg-white/90 shadow-sm">
        <CardHeader className="border-b border-slate-200">
          <div className="flex items-center justify-between gap-3">
            <div>
              <CardTitle className="text-xl text-slate-900">听力分析工作区</CardTitle>
              <CardDescription className="mt-2 text-sm leading-6 text-slate-600">
                这一步先把英语从“只有录题和写作”推进到“有第三条可直接使用的专属主链”。
              </CardDescription>
            </div>
            <Button
              type="button"
        ...
        <CardContent className="flex min-h-[680px] flex-col px-5 pb-5 pt-5">
          {latestAssistantMessage ? (
            <div className="mb-4">
              <StudyResultSummary
                ...
                title="本轮结果摘要"
                description="把最近一次听力回复提炼成结构化结果卡，方便快速回看主旨、定位和复练重点。"
              />
            </div>
          ) : null}
          ...
          <div className="mt-4 rounded-[24px] border border-sky-200 bg-sky-50/80 p-4 text-sm leading-6 text-slate-700">
            ...
            <p className="mt-2">
              这个模块先不做新的音频上传、播放器和转写链路，而是先把“听力题能被拆解、定位答案、复盘错因”的工作台建起来。
              等这条主链稳定，再决定是否补音频输入和复练播放器。
            </p>
          </div>
        </CardContent>
      </Card>
