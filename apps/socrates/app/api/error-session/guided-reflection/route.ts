...
function buildStudentSummary({
  ...
  const replay = answerMap.get('replay_error_moment') || '我当时是直接按习惯下手的。';
...
async function loadDiagnosis(sessionId: string, studentId: string) {
  const { data, error } = await supabase
    ...
    .select('id, session_id, student_id, root_cause_category, root_cause_statement, fix_actions, guided_reflection')
...
export async function GET(req: NextRequest) {
  try {
    ...
    const nextStep = reflection.completed
      ? null
      : GUIDED_REFLECTION_STEPS[Math.min(reflection.current_step, GUIDED_REFLECTION_STEPS.length - 1)];
  ...
  try {
    ...
    const normalizedStep =
      Number.isFinite(current_step) && Number(current_step) >= 0
        ? Math.min(Number(current_step), GUIDED_REFLECTION_STEPS.length - 1)
        : Math.min(reflection.current_step, GUIDED_REFLECTION_STEPS.length - 1);

    if (typeof student_answer !== 'string' || !student_answer.trim()) {
      return NextResponse.json({ error: 'student_answer is required' }, { status: 400 });
    }

    const stepDefinition = GUIDED_REFLECTION_STEPS[normalizedStep];
    ...
    const completed = nextStepIndex >= GUIDED_REFLECTION_STEPS.length;
    const updatedReflection: GuidedReflectionState = {
      current_step: completed ? GUIDED_REFLECTION_STEPS.length - 1 : nextStepIndex,
      completed,
      steps: nextSteps,
    ...
    const nextQuestion = completed ? null : GUIDED_REFLECTION_STEPS[nextStepIndex];
