...
export function buildStudyAssetExtractedText(asset: Pick<StudyAssetBridgeRecord, 'title' | 'summary' | 'module' | 'payload'>) {
  const payload = asset.payload || {};
  const lines: string[] = [];
...
export function buildStudyAssetFocusSummary(
  asset: Pick<StudyAssetBridgeRecord, 'id' | 'subject' | 'module' | 'title' | 'summary' | 'payload' | 'updated_at'>,
) {
  const payload = asset.payload || {};
  const resultSections = readStudyResultSummaryPayload(
    payload.resultSummary ?? payload.result_summary,
