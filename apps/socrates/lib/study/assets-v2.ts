...
export { buildStudyResultSummaryPayload, readStudyResultSummaryPayload };
...
export interface StudyAssetReviewBridgeResult {
  reviewHref: string;
  reviewId?: string;
...
export function buildStudyAssetReportHref(assetId: string) {
  return `/reports?focus_asset_id=${encodeURIComponent(assetId)}`;
}

export function buildStudyAssetPayloadWithSummary<T extends Record<string, unknown>>(
  payload: T,
  module: string,
  ...
  return {
    ...payload,
    resultSummary: buildStudyResultSummaryPayload(module, assistantContent),
  };
}
...
export async function bridgeStudyAssetToReview(params: {
  assetId: string;
  studentId: string;
}): Promise<StudyAssetReviewBridgeResult> {
  ...
  return {
    reviewHref: typeof data?.review_href === 'string' ? data.review_href : '',
    reviewId: typeof data?.review_id === 'string' ? data.review_id : undefined,
    errorSessionId: typeof data?.error_session_id === 'string' ? data.error_session_id : undefined,
