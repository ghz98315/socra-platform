...
import { ArrowLeft, ChevronLeft, ChevronRight, List, Lock, Settings, Moon, Sun, BookOpen, ScrollText } from 'lucide-react';
...
export default function BookReader() {
  ...
  // Calculate total pages for paged mode
  const calculatePages = () => {
    if (layoutMode === 'paged' && contentRef.current && containerRef.current) {
      const containerWidth = containerRef.current.clientWidth;
      const gap = window.innerWidth >= 768 ? 64 : 32; // 4rem = 64px, 2rem = 32px
      const totalWidth = contentRef.current.scrollWidth;
      // Use Math.round to avoid subpixel rendering issues causing an extra blank page
  ...
  useEffect(() => {
    ...
    // Observe content changes (like images loading) to recalculate pages
  ...
  }, [chapterId, layoutMode, fontSize, isLoaded]);

  // Reset page and scroll on chapter change
  useEffect(() => {
    setCurrentPage(0);
    window.scrollTo(0, 0);
  }, [chapterId]);
  ...
  const handlePrev = useCallback(() => {
    if (layoutMode === 'paged') {
      if (currentPage > 0) {
        ...
        if (prevChapter.isFree) navigate(`/read/${prevChapter.id}`);
        else navigate('/book');
      }
    }
  }, [layoutMode, currentPage, prevChapter, navigate]);
  ...
  const handleNext = useCallback(() => {
    if (layoutMode === 'paged') {
      if (currentPage < totalPages - 1) {
        ...
        if (nextChapter.isFree) navigate(`/read/${nextChapter.id}`);
        else navigate('/book');
      }
    }
  }, [layoutMode, currentPage, totalPages, nextChapter, navigate]);
  ...
  }, [handlePrev, handleNext, layoutMode]);
  ...
  if (!chapter) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-[#fafafa]">
        ...
        <Link to="/book" className="text-neutral-500 hover:text-neutral-900 underline">返回书籍主页</Link>
      </div>
    );
  }
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${theme === 'dark' ? 'bg-[#1a1a1a] text-[#e0ddd8] dark' : 'bg-[#fafafa] text-[#171717]'}`}>
      <SEO 
        title={`${chapter.title} | Socrates`}
        description={`阅读章节：${chapter.title}`}
      />
      ...
      <header className={`fixed top-0 left-0 right-0 backdrop-blur-md border-b z-50 transition-colors duration-300 ${theme === 'dark' ? 'bg-[#1a1a1a]/90 border-neutral-800' : 'bg-[#fafafa]/90 border-neutral-200'}`}>
        <div className="max-w-5xl mx-auto px-4 h-14 flex items-center justify-between">
          <Link to="/book" className={`flex items-center gap-2 transition-colors ${theme === 'dark' ? 'text-neutral-400 hover:text-neutral-200' : 'text-neutral-500 hover:text-neutral-900'}`}>
            <ArrowLeft className="w-4 h-4" />
            <span className="text-sm font-medium hidden sm:inline">返回目录</span>
          </Link>
          
          <div className="text-sm font-medium truncate max-w-[50%] text-center font-serif">
            {chapter.title}
          </div>
                      ...
                      if (c.isFree) {
                        return (
                          <button
                            ...
                            className={`text-left px-3 py-2.5 rounded-lg text-sm flex items-center justify-between transition-colors ${
                              isActive
                                ? (theme === 'dark' ? 'bg-[#e8600a]/20 text-[#e8600a]' : 'bg-[#fff5ee] text-[#e8600a]')
                                : (theme === 'dark' ? 'text-neutral-300 hover:bg-neutral-800' : 'text-neutral-700 hover:bg-neutral-100')
                            }`}
                          >
                        ...
                        return (
                          <div
                            key={c.id}
                            className={`text-left px-3 py-2.5 rounded-lg text-sm flex items-center justify-between ${
        ...
        <div className={`h-[2px] w-full absolute bottom-0 left-0 ${theme === 'dark' ? 'bg-neutral-800' : 'bg-neutral-100'}`}>
          <div 
            className="h-full bg-[#e8600a] transition-all duration-150 ease-out"
      ...
      >
        {/* Click zones for desktop/tablet */}
        {layoutMode === 'paged' && (
          <>
            ...
            <div className="py-12">
              {chapter.isPartCover ? (
                <div className="max-w-3xl mx-auto w-full py-12 md:py-24">
                  ...
                  <div className="text-[#e8600a] text-sm font-bold tracking-widest mb-6">
                    {chapter.partLabel}
                  </div>
                  ...
                  <p className={`text-lg md:text-xl mb-12 ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-600'}`}>
                    {chapter.partSubtitle}
                  </p>
                  <div className={`p-6 md:p-8 border-l-4 border-[#e8600a] ${theme === 'dark' ? 'bg-[#262626]' : 'bg-[#fff5ee]'}`}>
                    <p className={`text-base md:text-lg leading-relaxed text-justify ${theme === 'dark' ? 'text-neutral-300' : 'text-neutral-700'}`}>
                      {chapter.partSummary}
                    </p>
                  </div>
                </div>
                ...
                <>
                  <div className="mb-16 text-left">
                    ...
                    <span className="text-sm font-bold text-[#e8600a] tracking-widest mb-4 block">
                      {(() => {
                    ...
                    <h1 className={`text-3xl md:text-4xl font-serif font-bold leading-tight mb-8 ${theme === 'dark' ? 'text-[#f5f4f0]' : 'text-[#1a2744]'}`}>
                      {chapter.title}
                    </h1>
                    {chapter.summary && (
                      <div className={`p-6 border-l-4 border-[#e8600a] ${theme === 'dark' ? 'bg-[#262626]' : 'bg-[#f5f5f5]'}`}>
                        <p className={`text-base italic leading-relaxed text-justify ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}`}>
                          {chapter.summary}
                        </p>
                      </div>
                    )}
                  ...
                  <div 
                    className={`socrates-reader book-reader-content size-${fontSize}`}
                    dangerouslySetInnerHTML={{ __html: chapter.content || '<p>本章暂无内容，作者正在努力撰写中...</p>' }}
                  />
                </>
              )}
              ...
              <div className={`mt-20 pt-8 border-t flex flex-col sm:flex-row justify-between items-center gap-4 ${theme === 'dark' ? 'border-neutral-800' : 'border-neutral-200'}`}>
                {prevChapter && prevChapter.isFree ? (
                  <Link 
                    ...
                    className={`flex items-center gap-3 transition-colors w-full sm:w-auto justify-center sm:justify-start p-4 sm:p-0 rounded-xl ${theme === 'dark' ? 'text-neutral-400 hover:text-neutral-200 hover:bg-neutral-800 sm:hover:bg-transparent' : 'text-neutral-500 hover:text-[#1a2744] hover:bg-neutral-100 sm:hover:bg-transparent'}`}
                  >
                  ...
                  <div className="flex items-center gap-3 text-neutral-400 w-full sm:w-auto justify-center sm:justify-start p-4 sm:p-0">
                    ...
                    <div className="text-left">
                      ...
                      <div className="font-medium text-sm line-clamp-1 flex items-center gap-1">
                        <Lock className="w-3 h-3" /> {prevChapter.title}
                      </div>
                    </div>
                  </div>
                ) : (
                ...
                {nextChapter && nextChapter.isFree ? (
                  <Link 
                    ...
                    className={`flex items-center gap-3 transition-colors w-full sm:w-auto justify-center sm:justify-end p-4 sm:p-0 rounded-xl text-right ${theme === 'dark' ? 'text-neutral-400 hover:text-neutral-200 hover:bg-neutral-800 sm:hover:bg-transparent' : 'text-neutral-500 hover:text-[#1a2744] hover:bg-neutral-100 sm:hover:bg-transparent'}`}
                  >
                  ...
                  <div className="flex items-center gap-3 text-neutral-400 w-full sm:w-auto justify-center sm:justify-end p-4 sm:p-0 text-right">
                    <div className="text-right">
                      <div className="text-xs text-neutral-400 mb-1">下一章</div>
                      <div className="font-medium text-sm line-clamp-1 flex items-center justify-end gap-1">
                        {nextChapter.title} <Lock className="w-3 h-3" />
                      </div>
                    </div>
                    <ChevronRight className="w-5 h-5 shrink-0" />
                  </div>
            ...
            <div className="h-full w-full overflow-hidden py-8">
              {chapter.isPartCover ? (
                ...
                <div
                  ...
                  style={{
                    transform: `translateX(calc(-${currentPage * 100}% - ${currentPage * (isDesktop ? 4 : 2)}rem))`
                  }}
                  dangerouslySetInnerHTML={{
                    __html: `
                      <div class="mb-16 text-left" style="break-after: avoid;">
                        <div class="w-1 h-6 bg-[#e8600a] mb-6"></div>
                        <span class="text-sm font-bold text-[#e8600a] tracking-widest mb-4 block">
                          ${(() => {
                            if (chapter.chapterNumber === 0) return '序言';
                            if (chapter.chapterNumber === 13) return '附录';
                            const numMap = ['零', '一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二'];
                        ...
                        ${chapter.summary ? `
                          <div class="p-6 border-l-4 border-[#e8600a] ${theme === 'dark' ? 'bg-[#262626]' : 'bg-[#f5f5f5]'}">
                            <p class="text-base italic leading-relaxed text-justify ${theme === 'dark' ? 'text-neutral-400' : 'text-neutral-500'}">
                              ${chapter.summary}
                            </p>
                          </div>
                        ` : ''}
                      </div>
                      ${chapter.content || '<p>本章暂无内容，作者正在努力撰写中...</p>'}
                    `
                  }}
                />
              )}
      ...
      {layoutMode === 'paged' && (
        <footer className={`fixed bottom-0 left-0 right-0 h-16 backdrop-blur-md border-t z-50 flex items-center justify-between px-4 sm:px-8 transition-colors duration-300 ${theme === 'dark' ? 'bg-[#1a1a1a]/90 border-neutral-800' : 'bg-[#fafafa]/90 border-neutral-200'}`}>
          <div className="max-w-5xl mx-auto w-full flex items-center justify-between">
            <button 
              onClick={handlePrev} 
              className={`flex items-center gap-2 px-4 py-2 rounded-lg transition-colors ${theme === 'dark' ? 'hover:bg-neutral-800 text-neutral-300' : 'hover:bg-neutral-100 text-neutral-600'}`}
            >
              <ChevronLeft className="w-5 h-5" />
              <span className="hidden sm:inline">{currentPage === 0 ? '上一章' : '上一页'}</span>
            </button>
            
            <span className="text-sm font-medium text-neutral-500 font-mono">
