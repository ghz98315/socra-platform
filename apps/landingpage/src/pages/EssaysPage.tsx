...
export default function EssaysPage() {
  ...
  return (
    <>
      <SEO 
        title="错题管理心法系列文章 | Socrates"
        description="从错误开始，闭环学习。这里是 Socrates 的方法论沉淀，涵盖 8D 分析法、5 Why、PDCA 等工厂管理逻辑在学习上的应用。"
        canonical="https://socrates.socra.cn/essays"
      />
          ...
          {articles.map((article, index) => (
            ...
            >
              ...
              <div className="flex-grow">
                {article.slug !== '#' ? (
                  <Link to={`/essays/${article.slug}`} className="block group-hover:opacity-80 transition-opacity">
                ...
                {article.slug !== '#' && (
                  <Link to={`/essays/${article.slug}`} className="inline-flex items-center gap-1 text-sm font-medium text-neutral-900 hover:underline underline-offset-4">
