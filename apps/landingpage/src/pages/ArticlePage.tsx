...
export default function ArticlePage() {
  ...
  if (!article) {
    return (
      <div className="py-24 text-center">
        ...
        <Link to="/essays" className="text-neutral-500 hover:text-neutral-900 underline">返回系列目录</Link>
      </div>
    );
  }
  ...
  const nextArticle = articles.slice(currentIndex + 1).find(a => a.slug !== '#');

  return (
    <>
      <SEO 
        title={`${article.title} | Socrates`}
        description={article.excerpt}
        type="article"
        publishDate={article.date}
        canonical={`https://socrates.socra.cn/essays/${article.slug}`}
      />
      <article className="py-12 md:py-24 px-4 sm:px-6 max-w-3xl mx-auto">
        <Link to="/essays" className="inline-flex items-center gap-2 text-sm font-medium text-neutral-500 hover:text-neutral-900 mb-8 md:mb-12 transition-colors">
        ...
        <div className="prose prose-neutral md:prose-lg max-w-none text-neutral-700">
          ...
          <div className="mt-16 pt-8 border-t border-neutral-200 flex justify-between items-center">
            <div>
              ...
              <Link to="/essays" className="font-medium text-neutral-900 hover:underline flex items-center gap-2">
            ...
            {nextArticle && (
              <div className="text-right">
                ...
                <Link to={`/essays/${nextArticle.slug}`} className="font-medium text-neutral-900 hover:underline flex items-center gap-1">
