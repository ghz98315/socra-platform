...
const DEFAULT_CHAPTERS: BookChapter[] = [
  {
    id: 'prologue',
...
export function useBookChapters() {
  ...
  useEffect(() => {
    const saved = localStorage.getItem('socrates_book_chapters_v13');
    if (saved) {
      try {
      ...
      setChapters(DEFAULT_CHAPTERS);
      localStorage.setItem('socrates_book_chapters_v13', JSON.stringify(DEFAULT_CHAPTERS));
    }
    setIsLoaded(true);
  }, []);
  ...
  const updateChapter = (updatedChapter: BookChapter) => {
    ...
    localStorage.setItem('socrates_book_chapters_v13', JSON.stringify(newChapters));
  };
