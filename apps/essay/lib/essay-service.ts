...
function normalizeHighlights(input: unknown): Array<string | HighlightItem> {
  if (!Array.isArray(input)) return [];

  const out: Array<string | HighlightItem> = [];
  for (const item of input) {
    ...
    if (item && typeof item === "object") {
      ...
      if (dimension || description || anchorText) out.push({ dimension, description, anchorText });
      continue;
    }
  }
...
function normalizeMagicModification(input: unknown): MagicModification | undefined {
  if (!input || typeof input !== "object") return undefined;
  const raw = input as any;

  const originalPara = typeof raw.originalPara === "string" ? raw.originalPara.trim() : "";
  const upgradedPara = typeof raw.upgradedPara === "string" ? raw.upgradedPara.trim() : "";
  const secret = typeof raw.secret === "string" ? raw.secret.trim() : "";
  if (!originalPara && !upgradedPara && !secret) return undefined;
  return { originalPara, upgradedPara, secret };
}

function normalizeMaterialBox(input: unknown): MaterialBoxItem[] {
  if (!Array.isArray(input)) return [];

  const out: MaterialBoxItem[] = [];
  for (const item of input) {
    if (!item || typeof item !== "object") continue;
    const raw = item as any;

    const quote = typeof raw.quote === "string" ? raw.quote.trim() : "";
    const howToUse = typeof raw.howToUse === "string" ? raw.howToUse.trim() : "";
    if (!quote && !howToUse) continue;
    out.push({ quote, howToUse });
  }
...
export const analyzeEssay = async (
  ...
  try {
    ...
    const finalResult: EssayAnalysis = {
      ...
      magicModification: normalizeMagicModification(result.magicModification),
      goldenSentences: result.goldenSentences || [],
      materialBox: normalizeMaterialBox(result.materialBox),
